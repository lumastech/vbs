    ...
    <section class="bg-secondary-800 text-secondary-10 pt-9">
        <div class="mx-auto py-24 max-w-7xl">
                ...
                <div class="flex">
                    <form action="{{ url('searchorder') }}" method="post">
                        ...
                        <input type="text" name="order"
                            value="{{ isset($order)? $order->order_number:old('order')}}"
            ...
            <div class="border border-gray-600 p-4 md:p-9 rounded-md">
                ...
                <h2 class="text-yellow-600 mt-4 text-2xl md:text-4xl">Your Order</h2>
                ...
                <div class="flex-row gap-4">
                    <a href="{{url('downloadplanfile')}}/{{$order->plan->id}}/{{$order->id}}/file_1"
                        class="block my-2 py-1 px-2 rounded bg-yellow-600 text-white hover:bg-yellow-800 transition-all">Download
                        File 1</a>
                    @if($order->plan->file_2)<a
                        href="{{url('downloadplanfile')}}/{{$order->plan->id}}/{{$order->id}}/file_2"
                        class="block my-2 py-1 px-2 rounded bg-gray-600 text-white hover:bg-yellow-800 transition-all">Download
                        File 2</a>@endif
                    @if($order->plan->file_3)<a
                        href="{{url('downloadplanfile')}}/{{$order->plan->id}}/{{$order->id}}/file_3"
                        class="block my-2 py-1 px-2 rounded bg-gray-600 text-white hover:bg-yellow-800 transition-all">Download
                        File 3</a>@endif
                    @if($order->plan->file_4)<a
                        href="{{url('downloadplanfile')}}/{{$order->plan->id}}/{{$order->id}}/file_4"
                        class="block my-2 py-1 px-2 rounded bg-gray-600 text-white hover:bg-yellow-800 transition-all">Download
                        File 4</a>@endif
                </div>
                @endif
